...
function generate(ts, vueCompilerOptions, sourceTemplate, sourceLang, templateAst, hasScriptSetupSlots, cssScopedClasses = []) {
        ...
        if (node.type === 0 /* CompilerDOM.NodeTypes.ROOT */) {
            ...
            for (let i = 0; i < node.branches.length; i++) {
                ...
                if (branch.condition?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                    ...
                    appendFormattingCode(branch.condition.content, branch.condition.loc.start.offset, formatBrackets.normal);
            ...
            if (source.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                ...
                appendFormattingCode(source.content, source.loc.start.offset, formatBrackets.normal);
                codes.push(`) {\n`);
                writeInterpolationVarsExtraCompletion();
                for (const childNode of node.children) {
                    visitNode(childNode, parentEl);
    ...
    function visitElementNode(node, parentEl) {
        ...
        for (const [slotName, { nodes, slotDir }] of Object.entries(slotAndChildNodes ?? {})) {
            ...
            if (slotDir?.exp?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                ...
                appendFormattingCode(slotDir.exp.content, slotDir.exp.loc.start.offset, formatBrackets.normal);
            }
    ...
    function writeEvents(node, componentInstanceVar, componentCtxVar) {
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                writePropertyAccess((0, shared_1.camelize)('on-' + prop.arg.loc.source), // onClickOutside
                ...
                if (prop.arg.loc.source.startsWith('[') && prop.arg.loc.source.endsWith(']')) {
                    codes.push(`[(`);
                    writeInterpolation(prop.arg.loc.source.slice(1, -1), prop.arg.loc.start.offset + 1, capabilitiesPresets.all, '', '', prop.arg.loc);
                    codes.push(`)!]`);
                }
                else {
                ...
                appendFormattingCode(prop.exp.content, prop.exp.loc.start.offset, formatBrackets.normal);
                codes.push(`;\n`);
            }
    ...
    function writeChildren(node, parentEl) {
        const slotAndChildNodes = {};
        for (const child of node.children) {
            if (child.type === 3 /* CompilerDOM.NodeTypes.COMMENT */) {
                continue;
            }
            if (child.type !== 1 /* CompilerDOM.NodeTypes.ELEMENT */) {
                slotAndChildNodes.default ??= { nodes: [], slotDir: undefined };
                slotAndChildNodes.default.nodes.push(child);
            }
            else {
                const slotDir = child.props.find(prop => prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */ && prop.name === 'slot');
                const slotName = (slotDir?.arg?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */ && slotDir.arg.content) || 'default';
        ...
        for (const [slotName, { nodes, slotDir }] of Object.entries(slotAndChildNodes)) {
            ...
            if (!slotDir || !argRange) {
                ...
                writeObjectProperty(slotName, argRange, {
                ...
                codes.push(`[`);
                writeInterpolation(slotName, argRange[0] + 1, capabilitiesPresets.all, '', '', slotDir.loc.slot_name ?? (slotDir.loc.slot_name = {}));
                codes.push(`]`);
            }
            codes.push(': {} as any,\n');
        }
